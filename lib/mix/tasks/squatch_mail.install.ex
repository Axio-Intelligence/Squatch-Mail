...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.SquatchMail.Install do
    ...
    def igniter(igniter) do
      ...
      # No endpoint patching is needed for SNS raw-body capture:
      # `squatch_mail_dashboard`'s macro pipes the webhook route through
      # `SquatchMail.SNS.RawBodyPlug`, which reads the raw body itself (SNS
      # sends `text/plain`, which the host's `Plug.Parsers` passes through
      # unread — see `SquatchMail.Web.Router`'s "Webhook raw body" moduledoc).
      Igniter.add_notice(igniter, next_steps_notice(mount_dashboard?, dashboard_path))
    end
    ...
    defp next_steps_notice(mount_dashboard?, dashboard_path) do
      ...
      telemetry — no mailer changes required.
        * The SNS webhook route captures its own raw request body (for \
      signature verification) via SquatchMail.SNS.RawBodyPlug — no endpoint \
      changes required.
      """
    end
  end
