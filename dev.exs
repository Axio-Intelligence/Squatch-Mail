...
defmodule SquatchMailDev.Endpoint do
  ...
  # No `body_reader` needed for the SNS webhook: `squatch_mail_dashboard`
  # pipes that route through `SquatchMail.SNS.RawBodyPlug`, which captures the
  # raw body itself (SNS sends `text/plain`, which these parsers pass through
  # unread). This endpoint is a plain `mix phx.new`-style setup on purpose —
  # it demonstrates that a host needs to wire nothing for webhook signatures.
  plug Plug.Parsers,
